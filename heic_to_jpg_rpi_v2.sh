...
# Script per convertire file HEIC in JPG usando ImageMagick su Raspberrypi
# con directory di input e output specificate.

# Verifica se ImageMagick è installato
if ! command -v convert &>/dev/null; then
  echo "Errore: ImageMagick non è installato. Installarlo con: sudo apt install -y libpng-dev libjpeg-dev libtiff-dev imagemagick"
  exit 1
fi
